...
class SaveImages
{
    ...
    public function publish()
    {
        ...
        $old_image = $this->userAdmin->ImageProfile()->exists()  ? $this->userAdmin->ImageProfile :
                                                                   null;
        // HAS NEW IMAGE
        if (!empty($new_image)) {
    ...
    protected function makeUpload($new_image){
        $oldlocation = public_path($this->userAdmin->PathURLTemp . "/" . $new_image);
        $newlocation = public_path($this->userAdmin->PathURL ."/". $new_image);
    ...
    protected function deleteImage($old_image){
        ...
        // old image
        $old_image_url = $this->userAdmin->PathURL ."/" .$old_image;
    ...
    public function createImageDB($image_obj){
        ...
        // create image
        $this->userAdmin->ImageProfile()->create(['image'      => $image_obj['image'] ,
    ...
    public function updateImageDB($image_obj){
        ...
        // create image
        $this->userAdmin->ImageProfile()->update(['image'       => $image_obj['image'] ,
                                                  'user_id'    => $this->user->id ,
                                                  'user_ip'    => ip2long($_SERVER['REMOTE_ADDR']) ]);
    }
    ...
    public function deleteImageDB(){
        // create image
        $this->userAdmin->ImageProfile()->delete();
    }
}
